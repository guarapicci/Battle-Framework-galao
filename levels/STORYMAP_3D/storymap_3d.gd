...
## This script controls the story mode hub world.
## Dialogue sequences pause player input. 
## Stage entry platforms trigger story mode stages.
...
@onready var trigger_stage2: Area3D = %"Stage2 trigger"
@onready var main_player: Node3D = %"main player"
...
func _trigger_dialogue_stage_2():
	var dialogue_scene_root: DialogueSequence = dialogue_blueprint.instantiate()
	dialogue_scene_root.addSpeaker(["Sonic", "Standard", "Left", "Right", "Left"],
	["Emerl", "Standard", "Middle", "Right", "Left"],
	["Tails", "Standard", "Right", "Right", "Left"]
	)
	dialogue_scene_root.addDialogue("Looks like the egghead's\nbeen busy!","Sonic")
	dialogue_scene_root.changeSpeaker("Sonic", "Determined", "Right")
	dialogue_scene_root.changeSpeaker("Tails", "Determined", "Left")
	dialogue_scene_root.addDialogue("Badniks incoming!","Tails")
	add_child(dialogue_scene_root)
	dialogue_scene_root.process_mode = Node.PROCESS_MODE_ALWAYS
	_lock_all_interactive()
	await dialogue_scene_root.dialogue_ended
	print("dialogue is over. triggering match with stage 2 presets...")
	SceneChanger.change_scene_to_file("res://levels/STORY_1_2/STORY_1_2.tscn")
	pass
...
func _trigger_callback_stage2(body: Node3D) -> void:
	print("detected collision on stage 2 stand")
	if(body == main_player):
		trigger_stage1.monitoring = false
		_trigger_dialogue_stage_2()
		pass
	pass # Replace with function body.
